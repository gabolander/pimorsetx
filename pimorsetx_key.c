...
void center_header(char * message)
{
    move(1,(COLS-strlen(message))/2);
    printw(message);
    refresh();
}

void status_left(char * message)
{
    move(LINES-3,1);
    printw(message);
    refresh();
...
void status_right(char * message)
{
    move(LINES-3,COLS-strlen(message)-6);
    clrtoeol();
    move(LINES-3,COLS-strlen(message)-1);
    printw(message);
    clrtoeol();
    refresh();
}
...
//   digitalWrite(LedPin, HIGH); // Led off
  ...
  center_header("Waiting for keyboard input. Press 'Q' to quit.");
  while(1){
    c=getch();
    if(c>0) {
        // printf(" Pressed key (#%d) = %c\r\n",c,c);
        sprintf(msgtmp," Pressed key (#%d) = %c",c,c);
        status_left(msgtmp);
    }
    if(toupper(c)=='Q') break;
    res=0x00;
    // if(digitalRead(ButtonPin) == 0) { //indicate that button has pressed down
    if(c==KEY_DOWN){ //indicate that button has pressed down
...
//#ifdef DEBUG          
          ...
          sprintf(msgtmp,"- Microsecs spent in last SILENCE : %ld (millis=%ld)",micros_used,micros_used/1000L);
          status_right(msgtmp);
// #endif          
...
//       digitalWrite(LedPin, HIGH);
      if(Is_seq_started) {
        if(State_pressed) {
          ...
          sprintf(msgtmp,"+ Microsecs spent in last SIGN : %ld (millis=%ld)   ",micros_used,micros_used/1000L);
          status_right(msgtmp);
#endif
